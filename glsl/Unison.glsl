...
float saw(float freq, float time, float phase) {
	return fract(phase + freq * time / pi);
}

float drum(float freq, float time) {
	return clamp(sin(freq * time / pi) * exp(-4.0 * time) * 2.0, -1.0, 1.0) / 2.0;
}
...
vec2 noise(float time, float fade) {
	return vec2(hash(time * 512.0), hash(time * 1024.0)) * exp(-fade * time);
}
...
vec2 mainSound(int samp, float time) {
	...
	result /= 8.0 * spread;
	result += drum(noteFreq(drumNote), fract(time * 2.0));
	result += noise(fract(time + 0.5), 5.0) * 0.25;
	result += noise(fract(time * rhythm), 7.0) * 0.2;
	return result;
}